...
@section('content')
    <div class="mx-auto w-full max-w-lg px-4 sm:px-0">
        <a href="{{ route('books.index') }}" class="reset-link inline-block mb-4">← Back to Books</a>

        <div class="mb-4">
            <h1 class="mb-3 text-2xl font-semibold text-slate-900">Add New Book</h1>

            <form action="{{ route('books.store') }}" method="POST" class="space-y-5 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            @csrf
                <div class="space-y-2">
                    <label for="title" class="block text-sm font-medium text-slate-700">Názov</label>
                    <input type="text" name="title" id="title" required
                           class="block h-11 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
